...
import { Clock, Plus, Edit2, Trash2, Save, Calendar, FileText, ChevronDown, X } from 'lucide-react'
...
// Convert "9:30 AM" → "09:30" for <input type="time">
...
function ItemEditForm({ item, anchorRef, onSave, onCancel }) {
  ...
  useEffect(() => {
    ...
    setPos({ top: rect.bottom + 6 + window.scrollY, left: Math.max(8, left), width: Math.max(320, rect.width) })
  }, [anchorRef])
  ...
  return createPortal(
    ...
    >
      ...
      <div className="flex gap-2">
        <button
          onClick={() => onSave({ title: title.trim(), description: description.trim(), event_date: eventDate || null })}
          disabled={!title.trim()}
...
function TimeEditPopover({ item, anchorRef, onSave, onCancel }) {
  ...
  useEffect(() => {
    ...
    setPos({ top: rect.bottom + 6 + window.scrollY, left: Math.max(8, left) })
  }, [anchorRef])
  ...
  return createPortal(
    <div
      style={{ position: 'absolute', top: pos.top, left: pos.left, width: 260, zIndex: 9999 }}
...
export default function TimelineCalendar({ items, canEdit, onUpdate, onDelete, onAddAt }) {
  ...
  const [expandedId,  setExpandedId]  = useState(null)   // notes open
  const [timeEditId,  setTimeEditId]  = useState(null)   // quick time editor
  const [editingId,   setEditingId]   = useState(null)   // full edit form
              ...
              {scheduled.map(item => {
                ...
                return (
                  <div
                    ...
                    style={{ top: hoursToTop(hours) + 1 }}
                  >
                    {/* Event card */}
                    <div
                      className={`rounded-xl border-2 shadow-sm group transition-all duration-200 overflow-visible bg-white ${
                        isExpanded
                          ? 'border-cowc-gold ring-2 ring-cowc-gold/20'
                          : 'border-cowc-sand hover:border-cowc-gold/40'
                      }`}
                      style={{ minHeight: heightPx, cursor: 'pointer' }}
                        ...
                        <div className="flex-1 min-w-0">
                          {isShort ? (
                            <div className="flex items-center gap-2">
                              ...
                              {canEdit ? (
                                <button
                                  className="text-xs text-cowc-gold font-medium flex-shrink-0 whitespace-nowrap hover:underline focus:outline-none"
                            ...
                            <>
                              ...
                              {canEdit ? (
                                <button
                                  className="text-xs text-cowc-gold font-medium whitespace-nowrap hover:underline focus:outline-none mt-0.5 block"
                                  onClick={e => { e.stopPropagation(); setTimeEditId(item.id); setEditingId(null) }}
                                >
                                  {item.time} – {endTimeLabel(hours, dur)} · {durLabel(dur)}
                                </button>
                              ) : (
                                <span className="text-xs text-cowc-gold font-medium whitespace-nowrap mt-0.5 block">
                                  {item.time} – {endTimeLabel(hours, dur)} · {durLabel(dur)}
                                </span>
                              )}
                              {item.event_date && (
